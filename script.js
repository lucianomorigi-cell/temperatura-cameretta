...
function attendiStatoAuthIniziale() {
return new Promise(
(resolve) => {
...
form.style.border = "1px solid rgba(255,255,255,0.12)";
form.style.borderRadius = "22px";
form.style.background = "#121930";
form.style.boxShadow = "0 24px 70px rgba(0,0,0,0.45)";
form.style.color = "#f7f9ff";

form.innerHTML = `
<h2 style="margin:0 0 8px;font-size:1.5rem;">
Accesso
</h2>

<p style="margin:0 0 22px;color:#aab4cf;line-height:1.45;">
Inserisci le credenziali Firebase per accedere alla Camera Ascanio.
</p>

<label style="display:block;margin-bottom:14px;">
<span style="display:block;margin-bottom:7px;color:#aab4cf;">
Email
</span>
...
required
style="width:100%;height:46px;padding:0 12px;border:1px solid rgba(255,255,255,0.14);border-radius:12px;background:#0b1020;color:#fff;outline:none;"
>
</label>
...
<p
id="firebaseLoginErrore"
style="display:none;margin:0 0 14px;color:#ff8d98;line-height:1.4;"
></p>

<button
type="submit"
style="width:100%;min-height:50px;border:0;border-radius:14px;background:#5271ff;color:#fff;font-weight:800;cursor:pointer;"
>
ACCEDI
</button>
`;
...
const emailEl =
form.querySelector("#firebaseLoginEmail");

const passwordEl =
form.querySelector("#firebaseLoginPassword");

const erroreLoginEl =
form.querySelector("#firebaseLoginErrore");
...
bottone.textContent = "ACCESSO...";

erroreLoginEl.style.display = "none";
erroreLoginEl.textContent = "";
...
let utenteFirebase =
await attendiStatoAuthIniziale();

if (!utenteFirebase) {

utenteFirebase =
await mostraLoginFirebase();

}

console.log(
"[Auth] Pagina autenticata:",
utenteFirebase.uid
);
...
/* =========================================================
RIFERIMENTI FIREBASE OTTIMIZZATI
========================================================= */
...
/*
 * Contiene le funzioni unsubscribe restituite
 * dai listener Firebase.
 */
let listenerFirebaseAttivi = [];


/*
 * Cache locale degli ultimi campioni dello storico.
 */
let storicoLocale = new Map();

let timerAggiornamentoGrafico = null;
...
function convertiElementoStorico(
elemento
) {
...
/*
 * Evita di ridisegnare Chart.js centinaia di volte
 * durante il caricamento iniziale dei 720 campioni.
 */

function programmaAggiornamentoGrafico() {

if (
timerAggiornamentoGrafico !== null
) {
return;
}

timerAggiornamentoGrafico =
setTimeout(
() => {
...
/*
 * Gli ultimi 720 campioni vengono ricevuti all'apertura.
 * Successivamente Firebase invia solo i singoli figli
 * aggiunti, modificati o rimossi.
 */

function avviaListenerStorico() {
...
/*
 * Impedisce di creare listener doppi.
 */

if (
listenerFirebaseAttivi.length > 0
) {
return;
}

console.log(
"[Firebase] Listener realtime ATTIVI"
);
...
/* =========================================================
ULTIMO AGGIORNAMENTO ESP32
========================================================= */

const stopUltimoAggiornamento =
onValue(
...
const stopStorico =
avviaListenerStorico();


listenerFirebaseAttivi = [

stopTemperatura,

stopUmidita,

stopUltimoAggiornamento,
...
/* =========================================================
FERMA LISTENER FIREBASE
========================================================= */

function fermaListenerFirebase() {
...
console.log(
"[Firebase] Listener realtime DISATTIVATI"
);

listenerFirebaseAttivi.forEach(
(annullaListener) => {
...
/*
 * Se cambi scheda o mandi il browser in background,
 * vengono chiusi i listener Firebase.
 *
 * Quando torni sulla pagina vengono riaperti.
 */
...
/*
 * Questo controllo ogni secondo NON legge Firebase.
 * Serve solo a stabilire se l'ESP32 è ancora online
 * usando l'ultimo timestamp già ricevuto.
 */
...
/*
 * Avvia Firebase solo quando
 * la pagina è effettivamente visibile.
 */
